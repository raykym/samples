...
# 既に調べたIPは外して調査する mysqlへの負荷が高くなるはず
...
my $exp_sql = "select hostaddress from ipsearch_tbl where hostaddress = ?";
my $exp_sth = $db->prepare($exp_sql);
...
# 既に調査済みチェック
   $exp_sth->execute($host);
my $exp_res = $exp_sth->rows;

# chk_resが0ならばチェックを実行 
if ($chk_res == '0'&& $exp_res == '0') {

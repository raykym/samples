...
# テーブル作成、あればエラーになるが無視。
my $create = "CREATE TABLE ipsearch_tbl (hostaddress, timestamp, icmp, http, https)"; 

    $dblite->do($create);



my $sql = "INSERT INTO ipsearch_tbl(hostaddress, timestamp, icmp,http,https) VALUES (?,?,?,?,?)";
my $sth = $dblite->prepare($sql);
...
my $timestamp = time2str("%Y-%m-%d %H:%M:%S",time);
    ...
    # DBへの書き込み
        $sth->execute($host, $timestamp, $res1, $res2, $res3);
